...
CEPH_CONTROLLER_IP="192.168.0.65"
CLUSTERSCOPE="192.168.0.0/24"
SDS_PKG_URL="http://10.120.16.212/build/ThinkCloud-SDS/master/"
SDS_TAR_PKG=$(curl http://10.120.16.212/build/ThinkCloud-SDS/master/ | grep ThinkCloud-SDS-3.0.0-daily_ | tail -1 | sed 's/.*\(ThinkCloud-SDS-3.0.0-daily_[0-9]*_[0-9]*.tar.gz\).*/\1/')
SDS_PKG_DATE=$(echo ${SDS_TAR_PKG} | sed 's/.*ThinkCloud-SDS-3.0.0-daily_\([0-9]*_[0-9]*\).tar.gz.*/\1/')
#SDS_TAR_PKG="deployment-standalone-daily_20180814_89.tar.gz"

CEPH_NODE_HOST_USER="root"
CEPH_NODE_HOST_PASSWORD="yujiang2"

CEPH_NODE_NAME=(mira003 mira004 mira005 mira006)
CEPH_PUBLIC_IP=(192.168.0.60 192.168.0.61 192.168.0.62 192.168.0.63)
CEPH_CLUSTER_IP=(192.168.0.60 192.168.0.61 192.168.0.62 192.168.0.63)
CEPH_MANAGER_IP=(192.168.0.60 192.168.0.61 192.168.0.62 192.168.0.63)
